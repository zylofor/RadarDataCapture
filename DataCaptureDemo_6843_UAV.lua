-- 三发四收，针对93.8m-10FPS的测试
-- Pay Attention 使用enhance口
ar1.FullReset()
ar1.SOPControl(2)
...
file_path = string.gsub(file_path, "DataCaptureDemo_6843_UAV.lua","");
...
data_path     = "F:\\MyDataset\\DatasetFile\\UAV"
adc_data_path = "F:\\MyDataset\\DatasetFile\\UAV\\adc_data.bin"
...
---------------------------Channel & Adc Config---------------------------
-- Pay Attention 1表示启用天线，0表示关闭天线，三根发射天线4根接收天线，输出进制，保存进制，级联
...
if((partId == 1642) or (partId == 1843)) then
    if(ar1.ProfileConfig(0, 77, 30, 15, 255, 0, 0, 0, 0, 0, 0, 10, 1, 384, 6520, 0, 0, 48) == 0) then
...
elseif(partId == 6843) then
-- ProfileConfig~Pay Attention
-- 			   -,	启始频率,	休息时间 	ADC开始采样时间	chirp总时间 	-, -, -, -, -, -,	斜率常数,	-, ADC采样数量 	ADC采样频率kps 	-,	-, 	24~48信号增强
--    if(ar1.ProfileConfig(0,	60,		200,		6,		60, 		0, 0, 0, 0, 0, 0,	29.982, 	0, 128, 	10000, 		0, 	131072, 30) == 0) then
    if(ar1.ProfileConfig(0, 60.25, 10, 2, 118, 0, 0, 0, 0, 0, 0, 9.994, 0, 512, 6250, 0, 131072, 48) == 0) then
		WriteToLog("ProfileConfig Success\n", "green")
    else
        WriteToLog("ProfileConfig failure\n", "red")
    end
...
-------------------------------------ChirpConfig-------------------------------------
-- ChirpConfig~Pay Attention 启用发射填写，注释掉则不启用，需要和前面
...
-------------------------------------FrameConfig-------------------------------------
-- FrameConfig~Pay Attention
-- 默认配置
-- -, -, 总帧数 chirps/帧 ms/帧 -, -, -

if (ar1.FrameConfig(0, 2,  300, 255, 100,   0, 0, 1) == 0) then
